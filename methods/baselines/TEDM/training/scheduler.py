...
class ReduceLROnPlateauWithWarmup(object):
    ...
    def __init__(self, *args, mode='min', factor=0.1, patience=10,
        ...
        if args and isinstance(args[0], Optimizer):
            self.optimizer = args[0]
        else:
            self.optimizer = utils.instantiate_from_config(*args)

        if isinstance(min_lr, list) or isinstance(min_lr, tuple):
            if len(min_lr) != len(self.optimizer.param_groups):
                raise ValueError("expected {} min_lrs, got {}".format(
                    len(self.optimizer.param_groups), len(min_lr)))
    ...
    def _prepare_for_warmup(self):
        if self.warmup_lr is not None:
            if isinstance(self.warmup_lr, (list, tuple)):
                ...
                self.warmup_lrs = [self.warmup_lr] * \
                    len(self.optimizer.param_groups)
        else:
            self.warmup_lrs = None
        if self.warmup > self.last_epoch:
            if self.warmup_lrs is None:
                raise ValueError("warmup > 0 requires warmup_lr to be set")
...
class CosineAnnealingLRWithWarmup(object):
    ...
    def __init__(self, *args, T_max, last_epoch=-1, verbose=False,
    ...
    def _reduce_lr(self, epoch):
        for i, param_group in enumerate(self.optimizer.param_groups):
            progress = float(epoch - self.warmup) / \
                float(max(1, self.T_max - self.warmup))
    ...
    def _increase_lr(self, epoch):
        # used for warmup
        for i, param_group in enumerate(self.optimizer.param_groups):
            new_lr = float(param_group['lr']) + self.warmup_lr_steps[i]
            param_group['lr'] = new_lr
