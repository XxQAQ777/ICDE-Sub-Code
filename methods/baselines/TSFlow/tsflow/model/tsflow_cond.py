...
# torchtyping 0.1.5's runtime checker accesses ``Tensor.names``, an attribute
# removed by current PyTorch builds.  These annotations are documentation only;
# disable the incompatible development-time checker while retaining the model.
...
class TSFlowCond(TSFlowBase):
    def __init__(
        ...
        if setting == Setting.UNIVARIATE:
            ...
            # BackboneModel already implements its target_dim > 1 path for
            # multivariate cross-series processing.  This checkout referenced
            # a non-existent BackboneModelMultivariate symbol.
            self.backbone = BackboneModel(
                **backbone_params,
                num_features=num_features,
                target_dim=target_dim,
