...
# See tsflow_cond.py: torchtyping 0.1.5 runtime checks are incompatible with
# PyTorch 2.5, whereas the TensorType annotations remain useful documentation.
def typechecked(function):
    return function
...
class TSFlowBase(pl.LightningModule):
    ...
    def p_losses(
        ...
        if loss_mask is None:
            ...
            if loss_mask.dim() == predicted_flow.dim() - 1:
                loss_mask = loss_mask.unsqueeze(-1)
            if loss_mask.shape != predicted_flow.shape:
                raise ValueError(
                    "loss_mask must match the flow target shape; got {} vs {}".format(
                        tuple(loss_mask.shape), tuple(predicted_flow.shape)
                    )
                )
            squared_error = (dpsi - predicted_flow).square()
            valid = loss_mask.to(dtype=squared_error.dtype)
