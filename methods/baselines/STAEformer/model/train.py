...
def train(
    ...
    for epoch in range(max_epochs):
        ...
        # 只在特定 epoch 进行验证
        do_validate = (epoch + 1) % val_interval == 0 or epoch == max_epochs - 1

        if do_validate:
            val_loss = eval_model(model, valset_loader, criterion)
        else:
            # 非验证 epoch，保持上一次的验证值
            val_loss = val_loss_list[-1] if val_loss_list else np.inf

        val_loss_list.append(val_loss)

        # 输出日志：总是输出 Train Loss，只在验证时输出 Val Loss
        ...
        # Early stopping：只在有新验证结果时更新 wait 计数
        if do_validate:
            if val_loss < min_val_loss:
                ...
                wait += val_interval  # 修正：增加的应该是验证间隔数
                if wait >= early_stop * val_interval:
                    break
...
if __name__ == "__main__":
    ...
    seed = args.seed if args.seed is not None else torch.randint(1000, (1,)) # set random seed here
    ...
    if dataset.startswith("METRLA") or dataset.startswith("PEMSBAY"):
